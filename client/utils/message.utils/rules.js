...
module.exports = {
    async execute(interaction, isVerification, isEdit = false) {
        const embed1 = new EmbedBuilder()
            ...
            .setAuthor({ name: 'Sexuality And Gender Edicts', iconURL: config.images.gsa_icon })
            .setThumbnail(config.images.rules_thmb);

        const politics = isVerification ? '`#politics`' : await interaction.guild.channels.cache.find(channel => {return channel.name === 'politics' && !getChannelParentName(channel).includes('archive')}) ?? `\`#politics\``;
        ...
        const embed2 = new EmbedBuilder()
            ...
            .addFields(
                ...
                },
                {
                    ...
                    "value": `>>> Click the 3 bars on the top left of your screen, then click the 3 vertical dots next to SAGE @ ${process.env.SCHOOL_CODE}, then click \`\`\`Edit Per-server Profile\`\`\` and set your nickname.`,
                    "inline": true
                },
                {
                    "name": "How to change nickname on PC 💻",
                    "value": `>>> Click the dropdown menu on the top left where it says SAGE @ ${process.env.SCHOOL_CODE} and click \`\`\`Edit Per-server Profile\`\`\` and set your nickname.`,
                    "inline": true
                },
                {
                    "name": `‎\n3️⃣ Keep the server safe for work`,
                    "value": `We do not want sexual or otherwise explicit content here. Feel free to talk about sexuality, but refrain from being explicit. Many people have different comfort levels when it comes to talking about sex and other topics so please be mindful and use spoiler tags and warnings when necessary.`
                },
                {
                ...
                {
                    "name": `‎\n5️⃣ \`\`\`#general\-chat\`\`\` the main chat and should be used for casual discussions`,
                    "value": `Please keep topics of politics, trauma, mental health, and any sensitive topics in ${politics} and ${safeSpace}. Topics of politics should be talked about very carefully here. Please be respectful.`
                },
                {
            ...
            .setTimestamp()
            .setFooter({ text: isEdit ? `Updated` : `Posted`, iconURL: config.images.gsa_icon });
